...
/** 默认 preset id。 */
const DEFAULT_CHEAP_PRESET = "official-2026";
/** 默认时区（高峰窗口按此时区解释）。 */
const DEFAULT_CHEAP_TIMEZONE = "Asia/Shanghai";
/** 默认尾部余量（0 = 不做尾部保护，空闲窗口内都可开跑；建议按需设为 15 或 workerTimeoutMinutes）。 */
const DEFAULT_CHEAP_MARGIN_MINUTES = 0;
...
function parsePeakWindow(value) {
	if (typeof value !== "object" || value === null) return null;
	const record = value;
	const days = Array.isArray(record.days) ? [...new Set(record.days.filter((d) => typeof d === "number" && Number.isInteger(d) && d >= 0 && d <= 6))].sort((a, b) => a - b) : null;
...
*/
function parsePeakWindows(value) {
...
function formatPeakWindows(windows, timezone) {
...
function peakMask(windows) {
	const key = JSON.stringify(windows);
	const cached = maskCache.get(key);
	if (cached !== void 0) return cached;
	const mask = new Array(7 * 1440).fill(false);
	for (const window of windows) {
		...
		for (const day of window.days) {
			...
			for (let i = 0; i < span; i++) mask[(day * 1440 + start + i) % mask.length] = true;
		}
	}
...
function effectivePeakWindows(cfg) {
...
function effectiveCheapTimezone(cfg) {
...
*/
function effectiveMarginMinutes(cfg) {
...
function formatterFor(timeZone) {
	...
	try {
		...
		formatter = new Intl.DateTimeFormat("en-US", {
			...options,
			timeZone: DEFAULT_CHEAP_TIMEZONE
		});
	}
...
function localWeekMinutes(date, timeZone) {
	const parts = formatterFor(timeZone).formatToParts(date);
	let weekday = 0;
	let hour = 0;
	let minute = 0;
	for (const part of parts) if (part.type === "weekday") {
...
function isPeak(date, windows, timeZone) {
...
*/
function nextCheapStart(date, windows, timeZone) {
	...
	for (let delta = 1; delta <= mask.length; delta++) {
		...
		if (!current && previous) return new Date(date.getTime() + delta * 6e4);
	}
	return null;
}
/**
...
*/
function nextPeakStart(date, windows, timeZone) {
	const mask = peakMask(windows);
	const local = localWeekMinutes(date, timeZone);
	for (let delta = 1; delta <= mask.length; delta++) {
		const current = mask[(local + delta) % mask.length];
...
function formatDateTimeInTimezone(date, timeZone) {
	...
	if (year === "") return new Intl.DateTimeFormat("en-CA", {
		timeZone: DEFAULT_CHEAP_TIMEZONE,
		year: "numeric",
		month: "2-digit",
		day: "2-digit",
...
function evaluateCheapMode(cfg, now = /* @__PURE__ */ new Date()) {
	...
	if (isPeak(now, windows, timezone)) {
		const next = nextCheapStart(now, windows, timezone);
		const minutes = next === null ? 0 : Math.max(1, Math.round((next.getTime() - now.getTime()) / 6e4));
		return {
			cheapMode: true,
			inPeak: true,
			canRunNow: false,
			nextCheapStartAt: next?.toISOString() ?? "",
			nextCheapStartLabel: next === null ? "" : formatDateTimeInTimezone(next, timezone),
			waitReason: "peak",
			minutesUntilCheap: minutes,
			minutesUntilPeak: 0,
			marginMinutes: margin,
	...
	const minutesUntilPeak = upcomingPeak === null ? -1 : Math.max(0, Math.round((upcomingPeak.getTime() - now.getTime()) / 6e4));
...
function parse(raw) {
	...
	return {
		...
		peakWindows: parsePeakWindows(record.peakWindows) ?? (isKnownPreset(str(record.cheapPreset, d.cheapPreset)) ? CHEAP_PRESETS[str(record.cheapPreset, d.cheapPreset)].windows : d.peakWindows),
		...
		cheapMarginMinutes: clampInt(num(record.cheapMarginMinutes, d.cheapMarginMinutes), 0, 1440),
...
function parseQueuedTasks(value) {
...
var DispatcherStore = class {
	...
	async patch(args) {
		...
		if (args !== void 0 && typeof args.cheapMode === "boolean") next.cheapMode = args.cheapMode;
		if (args !== void 0 && typeof args.cheapStrategy === "string" && (args.cheapStrategy === "wait" || args.cheapStrategy === "skip")) next.cheapStrategy = args.cheapStrategy;
		if (args !== void 0 && args.cheapMarginMinutes !== void 0) next.cheapMarginMinutes = clampInt(Number(args.cheapMarginMinutes), 0, 1440);
		if (args !== void 0 && typeof args.cheapPreset === "string" && args.cheapPreset.trim() !== "") {
			...
			} else next.cheapPreset = "custom";
		}
		if (args !== void 0 && Array.isArray(args.peakWindows)) {
			const parsed = parsePeakWindows(args.peakWindows);
			if (parsed !== null) {
				next.peakWindows = parsed;
				next.cheapPreset = "custom";
			}
		}
		if (args !== void 0 && typeof args.peakWindowsText === "string") {
		...
		if (args !== void 0 && typeof args.cheapTimezone === "string" && args.cheapTimezone.trim() !== "") next.cheapTimezone = args.cheapTimezone.trim();
		await this.save(next);
		return this.view();
	}
	/** 把任务合并进省钱模式排队队列，并记录下个空闲开始时刻（落盘）。 */
	async enqueueCheap(tasks, nextCheapStartAt) {
		const cfg = await this.load();
		const queuedAt = (/* @__PURE__ */ new Date()).toISOString();
		const merged = /* @__PURE__ */ new Map();
		for (const task of cfg.cheapQueue) merged.set(task.id, task);
		for (const task of tasks) merged.set(task.id, {
			...task,
			queuedAt
		});
		await this.save({
			...cfg,
			cheapQueue: [...merged.values()],
			nextCheapStartAt
		});
		return this.view();
	}
	/** 清空排队队列（到点执行 / 关闭省钱模式 / 手动绕过时调用）。 */
	async clearCheapQueue() {
		const cfg = await this.load();
		await this.save({
			...cfg,
			cheapQueue: [],
...
function mergeTasks(queue, tasks) {
	const merged = /* @__PURE__ */ new Map();
...
function queuedNotice(tasks, decision) {
	const lines = ["💰 省钱模式：已排入 " + (decision.nextCheapStartLabel || decision.nextCheapStartAt) + " 空闲时段执行 · " + tasks.length + " 项", ...tasks.slice(0, 12).map((t) => "- " + t.title)];
	if (tasks.length > 12) lines.push("…还有 " + (tasks.length - 12) + " 项");
	lines.push("（" + decision.reason + "）");
	return lines.join("\n");
}
/** Build the "skipped this round" notification body. */
function skippedNotice(tasks, decision) {
	const lines = ["💰 省钱模式：当前为高峰时段，本轮跳过 " + tasks.length + " 项（策略=skip）", ...tasks.slice(0, 12).map((t) => "- " + t.title)];
	if (tasks.length > 12) lines.push("…还有 " + (tasks.length - 12) + " 项");
	lines.push("（" + decision.reason + "）");
	return lines.join("\n");
}
...
*/
async function runAutoExecuteGated(store, api, tasks, opts = {}) {
	const cfg = await store.load();
	const log = [];
	const base = {
		mode: "disabled",
	...
	const runOpts = {
		...
		...opts.onComplete !== void 0 ? { onComplete: opts.onComplete } : {},
		...opts.notifyResult !== void 0 ? { notifyResult: opts.notifyResult } : {},
		...opts.notify !== void 0 ? { notify: opts.notify } : {}
	};
	const bypass = opts.ignoreCheapMode === true;
	if (cfg.cheapMode && !bypass) {
		const decision = evaluateCheapMode(cfg, opts.now ?? /* @__PURE__ */ new Date());
		if (!decision.canRunNow) {
			...
			return {
				...
				mode: "queued",
				queued: queuedCount,
				nextCheapStartAt: decision.nextCheapStartAt,
				nextCheapStartLabel: decision.nextCheapStartLabel,
				inPeak: decision.inPeak,
				decision
			};
		}
	...
	return {
		...
		outcome,
		...countsOf(outcome),
		queued: (await store.view()).cheapQueueCount,
		log
	};
}
/**
...
*/
async function flushCheapQueueIfDue(store, api, opts = {}) {
	const cfg = await store.load();
	...
	return defineTool$1({
		...
		async execute() {
			try {
				...
				const decision = evaluateCheapMode(await ctx.store.load(), /* @__PURE__ */ new Date());
				...
				if (view.cheapMode) {
					...
					parts.push("省钱策略：" + (view.cheapStrategy === "skip" ? "本轮跳过" : "排队等到下个空闲开始") + " · 尾部余量 " + view.cheapMarginEffectiveMinutes + " 分钟" + (view.cheapMarginMinutes === 0 ? "（0 = 关闭尾部保护）" : ""));
					...
					parts.push("排队任务：" + view.cheapQueueCount + " 项" + (view.cheapQueueCount > 0 ? "（" + view.cheapQueue.map((t) => t.title).join("；") + "）" : ""));
				} else {
...
function dispatcherConfigTool(ctx) {
	return defineTool$1({
		...
		parameters: {
			...
			cheapMode: {
				type: "boolean",
				description: "省钱模式：自动执行只在 DeepSeek 空闲（优惠）时段开跑（默认 false）"
			},
			cheapPreset: {
				type: "string",
				description: "时段口径 preset：official-2026（官方现行）/ legacy-utc（旧版 UTC 16:30–00:30）/ custom"
			},
			peakWindowsText: {
				type: "string",
				description: "自定义高峰时段，每行 `1,2,3,4,5 09:00-12:00`（0=周日…6=周六；start>end 跨午夜；空=一直空闲）"
			},
			cheapTimezone: {
				type: "string",
				description: "解释高峰时段的 IANA 时区（默认 Asia/Shanghai）"
			},
			cheapStrategy: {
				type: "string",
				enum: ["wait", "skip"],
				description: "高峰期策略：wait=排队等到下个空闲开始；skip=本轮跳过"
			},
			cheapMarginMinutes: {
				type: "number",
				description: "尾部余量分钟：距下个高峰不足该值时不开跑，排到下个空闲段；0=关闭尾部保护（默认）；建议设为 15 或 workerTimeoutMinutes"
			},
			reset: {
				type: "boolean",
				description: "恢复默认配置"
		...
		async execute(args) {
			try {
				...
				return {
					ok: true,
					message: "配置已保存：" + (view.enabled ? "启用" : "禁用") + " · " + interval + auto + timeout + workspace + cheap + " · 来源「" + view.projectName + "」" + testLine + " · 通知：" + ([
					...
					cheapMode: view.cheapMode,
					cheapPreset: view.cheapPreset,
					cheapStrategy: view.cheapStrategy,
					cheapMarginMinutes: view.cheapMarginMinutes,
					cheapMarginEffectiveMinutes: view.cheapMarginEffectiveMinutes,
					cheapTimezone: view.cheapTimezone,
					peakWindowsText: view.peakWindowsText,
					nextCheapStartAt: view.nextCheapStartAt,
					cheapQueueCount: view.cheapQueueCount,
...
function dispatcherRunTool(ctx) {
	return defineTool$1({
		...
		parameters: { ignoreCheapMode: {
			type: "boolean",
			description: "绕过省钱模式，立刻执行（默认 false：高峰时段排入空闲时段或跳过）"
		} },
		output: {
			schema: {
				type: "object",
		...
		async execute(args) {
			try {
				const ignoreCheapMode = args !== void 0 && args.ignoreCheapMode === true;
				...
				if (result.ok && result.tasks.length > 0) {
					if ((await ctx.store.load()).autoExecute) {
						const gated = await runAutoExecuteGated(ctx.store, ctx.api, result.tasks, {
							notifyResult: true,
							ignoreCheapMode
						});
						autoMode = gated.mode;
				...
				const execLine = autoMode === "queued" ? " 省钱模式：已排入 " + (nextCheapStartAt || "下个空闲时段") + " 的空闲时段执行（排队 " + cheapQueued + " 项）。" : autoMode === "skipped" ? " 省钱模式：高峰时段，本轮跳过。" : autoExecuted > 0 ? " 自动执行 " + autoExecuted + " 项，完成 " + autoCompleted + " 项。" : "";
				return {
					ok: result.ok,
...
function makeRoutes(deps) {
	...
	*/
	const statusPayload = async (source) => {
		const [view, cfg] = [await source.view(), await source.load()];
		const decision = evaluateCheapMode(cfg, /* @__PURE__ */ new Date());
		return {
			...view,
			inPeakNow: decision.inPeak,
			cheapCanRunNow: decision.canRunNow,
	...
	return [
		...
		},
		{
			...
			handler: async (req, res) => {
				if (!guard(req, res, "POST")) return;
				try {
					const ignoreCheapMode = (await readJsonBody(req))?.ignoreCheapMode === true;
					const result = await doDispatch(store, api);
					let cheap;
					if (result.ok && result.tasks.length > 0) {
						if ((await store.load()).autoExecute) {
							const gated = await runAutoExecuteGated(store, api, result.tasks, {
								notifyResult: true,
								ignoreCheapMode
							});
							cheap = {
					...
					writeJson(res, 200, cheap === void 0 ? result : {
						...result,
						cheap
					});
				} catch (error) {
			...
			(async () => {
				...
				try {
					...
					if (cfg.autoExecute && result.tasks.length > 0) {
						...
						const counts = "executed=" + gated.executed + " completed=" + gated.completed + " failed=" + gated.failed + " queued=" + gated.queued;
...
//#endregion
export { CHEAP_PRESETS, DEFAULT_CHEAP_MARGIN_MINUTES, DEFAULT_CHEAP_PRESET, DEFAULT_CHEAP_TIMEZONE, DEFAULT_CONFIG_FILE, DEFAULT_TASK_FILE, DEFAULT_WORKER_PROMPT, DEFAULT_WORKER_TIMEOUT_MINUTES, DEFAULT_WORKER_TIMEOUT_MS, DEFAULT_WORKSPACE_STORE, DISPATCHER_API, DISPATCHER_GUIDANCE, DeferredController, DispatcherStore, HASH_SAFE, LEGACY_UTC_WINDOWS, OFFICIAL_2026_WINDOWS, TIMER_LABEL, WECHAT_GATEWAY_URL, apply, buildFlomoContent, buildTools, buildWorkerPrompt, bundledScriptPath, coerceQueue, configPath, defaultQueue, defineTool, dispatcherConfigTool, dispatcherReportTool, dispatcherRunTool, dispatcherStatusTool, doDispatch, dshHome, effectiveCheapTimezone, effectiveMarginMinutes, effectivePeakWindows, escapeHashes, evaluateCheapMode, flomoMemo, flushCheapQueueIfDue, formatDateTimeInTimezone, formatDays, formatPeakWindowLine, formatPeakWindows, formatPeakWindowsText, hhmmToMinutes, inject, installedScriptPath, isKnownPreset, isPeak, listWorkspaces, localDateString, localWeekMinutes, macNotify, makeRoutes, minutesToHhmm, name, newestSessionMtime, nextCheapStart, nextPeakStart, normalizePreset, notifyText, parsePeakWindow, parsePeakWindows, parsePeakWindowsText, peakMask, pluginPath, queuePath, resolveWorkspacePath, resolveWorkspaceTitle, runAutoExecute, runAutoExecuteGated, spawnWorker, summarizeWorkerOutput, timerPlistPath, wechatRecipient, wechatSend, wechatStateDir, workspaceStorePath };
